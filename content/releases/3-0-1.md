...
v: "3.0.1"
images:
  - /images/og/graphic-rhize-release-v3.0.1.png
aliases:
  - /latest/releases/3-0-1
...
- [Message events]({{< relref "../latest/how-to/bpmn/bpmn-elements/#events" >}}) now support topics that use the `+` wildcard.
- Add configuration option to set __traceDebug flag on all BPMN executions. For details, read [Debug workflows]({{< relref "../latest/how-to/bpmn/debug-workflows" >}}).
- Allow `__traceDebug` to be set from [variable context]({{< relref "../latest/how-to/bpmn/create-workflow/#access-process-variable-context" >}}).
- Add support for querying a range of [work calendar]({{< relref "../latest/how-to/work-calendars" >}}) entries for a Work Calendar Definition.
...
**Change**
- Reduce number of retries in [BPMN recovery]({{< relref "../latest/deploy/maintain/bpmn-nodes/" >}}), speeding up response times when node does not exist.
From the user's perspective, the delay was particularly notable on [API BPMN triggers]({{< relref "../latest/how-to/bpmn/trigger-workflows/#start-a-workflow-from-an-api" >}}) when the specified workflow did not exist.
...
3. Configure the audience and role for the Users or groups that should be able to run BPMN workflows. For details, read [Install Keycloak]({{< relref "../latest/deploy/install/keycloak/" >}}).
...
Read [About calendars and overrides]({{< relref "../latest/how-to/work-calendars/about-calendars-and-overrides" >}}) for details about how the service and its relationships work.
...
1. Query for your `StateTransitionInstance`:
   ...
   ```
2. Use a [mutation]({{< relref "../latest/how-to/gql/mutate/#update" >}}) to update these to have an ID and label.
