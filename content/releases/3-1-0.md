...
The new scope map features rules and jurisdictions. Jurisdictions can be used to create a group of permissions for certain entities within a hierarhcy scope. This is useful for allowing contract manufacturing organizations to read/write to the data hub, without being able to see the entire manufacturing knowledge graph.
...
- `quantityUnitOfMeasure` has been deprecated in favor of `quantityUoM` for MaterialLot and MaterialSubLot in order to standardize variable name across the schema. While `quantityUnitOfMeasure` is deprecated it will continue to work and will be removed in a future releaase. To migrate before the `quantityUnitOfMeasure` is completely removed, ensure all clients are now using `quantityUoM`:
...
- `DispatchStatus` has been changed to match ISA-95 standard set. Existing status' which didn't align are now deprecated. The will continue to work but will be removed in a future release. It is recommended to migrate clients to the new set.
