...
- Fix duplicate personnel class draft being created on stat.
...
- Fix multiple HTML5 backend error by adding DndManager to ArboristTree usage.
...
- Change permissions file structure from `map[string][]interface{}` to a known structure for resource authorization rules
...
- Fix issue where Agent would panic if it's config didn't have an active version.
...
To upgrade to v3.1.0 from v3.0.3, first ensure that you have made corrections to any breaking changes.

Rhize BAAS now requires a rule with the `admin` role for specific actions, such as setting up the schema. Ensure that your scopemap contains the following rule in some form, with "admin" being case-sensitive:
...
Rhize services now use YAML instead of JSON for their configuration files. This is represented in Helm overrides by the object `rhize<SERVICE>Config` (i.e. `rhizeAuditConfig`). Charts for v3.1.0 should only create YAML config files, however there may be differences in the keys for certain configuration options. Consult the values file for each Helm chart and the changelog for specific changes as necessary.

Various schema changes might cause breaking changes during upgrade. A tool will be created shortly to aid in migration, however you can correct these issues manually by modifying and re-uploading the data.

For the specific breaks:

- `quantityUnitOfMeasure` has been changed to `quantityUoM` for MaterialLot and MaterialSubLot, to standardize variable name across the schema. While `quantityUnitOfMeasure` is deprecated, any data using it will need to be updated. To fix:
...
- `DispatchStatus` has been changed such that several states are now deprecated, to match ISA-95 documentation. This change is the least likely to cause any specific issues, but it is recommended to query for any issues after upgrade.
...
- The `id` field for `OperationsEventDefinitionRecordSpecification` has been changed to use the `@id` directive. Each `id` must now be unique, thereby any duplicate ids will cause issues. To fix:
    ...
    1. Query all `id` for `OperationsEventDefinitionRecordSpecification`, and store information.
