...
To use [upsert operations]({{< relref "old/howto/upserts.md">}}) on a
...
Dgraph supports a number of [RDF types in mutations]({{< relref "old/mutations/language-rdf-types.md" >}}).

As well as implying a schema type for a [first mutation]({{< relref "old/query-language/schema.md" >}}), an RDF type can override a schema type for storage.
...
{{% notice "note" %}}Filtering on a predicate by applying a [function]({{< relref "old/query-language/functions.md" >}}) requires an index.{{% /notice %}}
...
* Indexes can be applied on predicates which have a list type and you can use [Functions]({{<relref "old/query-language/functions.md">}}) on them.
