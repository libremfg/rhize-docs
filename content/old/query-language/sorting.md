...
For sorting on predicates with [sortable indices]({{< relref "old/query-language/schema.md#sortable-indices">}}), Dgraph sorts on the values and with the index in parallel and returns whichever result is computed first.
...
{{% notice "tip" %}}
Sorted queries retrieve up to 1000 results by default. This can be changed with [first]({{< relref "old/query-language/pagination.md#first">}}).
{{% /notice %}}
