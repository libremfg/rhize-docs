...
Facets keys and values can use language-specific characters directly when mutating. But facet keys need to be enclosed in angle brackets `<>` when querying. This is similar to predicates. See [Predicates i18n]({{< relref "old/query-language/schema.md#predicates-i18n" >}}) for more info.
...
Facets on UID edges can be stored in [value variables]({{< relref "old/query-language/value-variables.md" >}}).  The variable is a map from the edge target to the facet value.
...
Facet values of `int` and `float` can be assigned to variables and thus the [values propagate]({{< relref "old/query-language/value-variables.md#variable-propagation" >}}).
